...
// Ensure the Vite dev server is up before launching tests.
...
async function test(name, fn) {
  ...
  const prevTimeout = page ? page.defaultTimeout : 30000;
  if (page) page.setDefaultTimeout(5000);
  try {
    await fn();
    ...
    if (page) page.setDefaultTimeout(prevTimeout);
...
async function start() {
  // Make sure the Vite server is up
  log('  Waiting for Vite dev server at ' + APP_URL + ' ...');
  const up = await ensureServer(APP_URL);
  if (!up) {
    throw new Error(`Vite dev server not reachable at ${APP_URL}. Run \`npm run dev\` in another terminal.`);
  }
  log('  Server is up.');
  ...
  // Always start from a clean slate so we know exactly what seed data to expect.
...
(async () => {
  ...
  await test('Tasks calendar view', async () => {
    await page.locator('button[data-view="cal"]').click();
    await page.waitForTimeout(200);
    const bars = await page.locator('.gantt-bar').count();
  ...
  await test('Drag task from backlog to in_progress (simulated)', async () => {
    ...
    // Force a full page reload so the in-memory state is re-read from localStorage
    await page.reload({ waitUntil: 'load' });
  ...
  await test('Charge all members', async () => {
    await page.locator('button[data-view="overview"]').click();
    await page.waitForTimeout(200);
    const btn = await page.locator('#charge-all');
  ...
  await test('Open new message form', async () => {
    await page.click('button:has-text("Neue Mitteilung")');
    await page.waitForSelector('.modal-title', { timeout: 1500 });
